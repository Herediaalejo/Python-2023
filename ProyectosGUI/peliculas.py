...
class Aplicacion():
    ...
    def createWidgets(self):
        ...
        title_input = ttk.Entry(self.window)
        title_input.grid(row=1,column=0, sticky=N)
        movies_box = Listbox(self.window, width=25)
        movies_box.grid(row=1, column=1, rowspan=2)
        add_button = ttk.Button(self.window, text="Añadir", command=lambda :[movies_box.insert(END, title_input.get()), title_input.delete(0, END)])
        add_button.grid(row=1,column=0, pady=30)
        clear_button = ttk.Button(self.window, text="Borrar todo", command=lambda:movies_box.delete(0,END)).grid(row=1, column=0, sticky=S)
